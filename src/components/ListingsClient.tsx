    ...
    <div className="max-w-6xl mx-auto px-5 sm:px-8 py-10">
      ...
      <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
        <p className="text-[#5A5A78] text-[14px]">
          Menampilkan{" "}
          <span className="font-semibold text-[#141422]">
            {filtered.length}
          </span>{" "}
          properti
        </p>
      ...
      {showFilter && (
        <div className="bg-white border border-[#E4E4F0] rounded-2xl p-5 mb-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {/* Tipe */}
          <div>
            <label className="text-[12px] font-semibold text-[#5A5A78] mb-1 block">
              Tipe
            </label>
            <select
              value={type}
              onChange={(e) => setType(e.target.value)}
              className="w-full border border-[#E4E4F0] rounded-lg px-3 py-2 text-[13px] focus:outline-none focus:border-[#343270]"
            >
      ...
      {filtered.length > 0 ? (
        ...
        <div className="text-center py-24 text-[#8E8EA8]">
          ...
          <button
            onClick={resetFilters}
            className="mt-4 px-5 py-2.5 rounded-xl bg-[#343270] text-white text-[13px] font-semibold hover:bg-[#2E9AB8] transition-colors"
          >
            Reset Filter
          </button>
        </div>
