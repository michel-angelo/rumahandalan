...
async function getFeaturedProperties() {
  const { data } = await supabase
    .from('properties')
    .select('*, cluster:clusters(*), location:locations(*), images:property_images(*)')
    .eq('is_featured', true)
    .eq('status', 'tersedia')
    .limit(6)
...
const statusConfig: Record<string, { label: string; className: string }> = {
  ...
  booked:       { label: 'Booked',       className: 'bg-orange-50 text-orange-600 border border-orange-200' },
}
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        ...
        {(property.is_kpr || property.is_cash_keras || property.is_subsidi) && (
          <div className="flex flex-wrap gap-1.5">
            {property.is_kpr && (
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-md ${paymentBadgeClass}`}>KPR</span>
            )}
            {property.is_cash_keras && (
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-md ${paymentBadgeClass}`}>Cash Keras</span>
            )}
            {property.is_subsidi && (
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200">Subsidi</span>
            )}
          </div>
        )}
  ...
  return (
    ...
    >
      ...
      <div className="relative p-5">
        ...
        <h3 className="font-serif text-white text-[18px] font-bold leading-tight">{cluster.name}</h3>
        <p className="text-[#AADDE9] text-[13px] mt-0.5">{cluster.developer}</p>
        {cluster.description && (
          <p className="text-white/70 text-[12px] mt-2 line-clamp-2">{cluster.description}</p>
        )}
      </div>
    </Link>
  )
    ...
    <div className="bg-white rounded-2xl border border-[#E4E4F0] p-6 flex flex-col gap-4">
      <div className="flex gap-1">
        {Array.from({ length: 5 }).map((_, i) => (
          <svg key={i} className={`w-4 h-4 ${i < (testimonial.rating ?? 5) ? 'text-amber-400' : 'text-[#E4E4F0]'}`} fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-[#F0F0F8]">
        ...
        <div>
          <p className="text-[13px] font-semibold text-[#141422]">{testimonial.name}</p>
          {testimonial.property_bought && (
            <p className="text-[11px] text-[#8E8EA8]">{testimonial.property_bought}</p>
          )}
        </div>
      </div>
    </div>
...
export default async function Home() {
  const [featuredProperties, promoClusters, heroImage, testimonials] = await Promise.all([
  ...
  return (
    <div className="bg-[#F7F7FB] min-h-screen">
        ...
        <div className="relative w-full max-w-6xl mx-auto px-5 sm:px-8 py-24">
          <div className="max-w-2xl">
            ...
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-4 py-1.5 mb-6">
              ...
              <span className="text-white/90 text-[13px] font-medium">Properti Terpercaya di Depok</span>
            </div>

            <h1 className="font-serif text-white text-4xl sm:text-5xl md:text-6xl font-bold leading-[1.1] mb-5">
              Temukan Hunian<br />
              <span className="text-[#2E9AB8]">Impian Anda</span><br />
              di Depok
            </h1>
            <p className="text-white/70 text-[16px] sm:text-[17px] leading-relaxed mb-10 max-w-xl">
              Rumah Andalan hadir membantu Anda menemukan properti terbaik sesuai kebutuhan dan budget.
            </p>
          </div>
      ...
      {promoClusters.length > 0 && (
        <section className="max-w-6xl mx-auto px-5 sm:px-8 py-16 md:py-20">
          <div className="flex items-end justify-between mb-8">
            <div>
              <p className="text-[#2E9AB8] text-[13px] font-semibold uppercase tracking-widest mb-2">Penawaran Terbatas</p>
              <h2 className="font-serif text-[#141422] text-3xl md:text-4xl font-bold">Cluster Promo</h2>
            </div>
            <Link href="/clusters" className="hidden sm:inline-flex items-center gap-1.5 text-[#343270] text-[14px] font-semibold hover:text-[#2E9AB8] transition-colors">
              Lihat Semua
      ...
      <section className="max-w-6xl mx-auto px-5 sm:px-8 py-16 md:py-20">
        <div className="flex items-end justify-between mb-8">
          <div>
            <p className="text-[#2E9AB8] text-[13px] font-semibold uppercase tracking-widest mb-2">Pilihan Terbaik</p>
            <h2 className="font-serif text-[#141422] text-3xl md:text-4xl font-bold">Properti Unggulan</h2>
          </div>
          <Link href="/listings" className="hidden sm:inline-flex items-center gap-1.5 text-[#343270] text-[14px] font-semibold hover:text-[#2E9AB8] transition-colors">
            Lihat Semua
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
        ...
        <div className="mt-8 text-center sm:hidden">
          <Link href="/listings" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-[#343270] text-white text-[14px] font-semibold hover:bg-[#2E9AB8] transition-colors">
      ...
      <section className="bg-[#1E1E40] py-16 md:py-24">
        <div className="max-w-6xl mx-auto px-5 sm:px-8">
          <div className="text-center mb-12">
            <p className="text-[#2E9AB8] text-[13px] font-semibold uppercase tracking-widest mb-2">Keunggulan Kami</p>
            <h2 className="font-serif text-white text-3xl md:text-4xl font-bold">Kenapa Rumah Andalan?</h2>
          </div>
            ...
            {[
              ...
              <div key={title} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-colors">
                <div className="w-12 h-12 rounded-xl bg-[#285090]/40 border border-[#285090]/40 flex items-center justify-center mb-4">
                  <svg className="w-6 h-6 text-[#2E9AB8]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    {icon}
                  </svg>
                </div>
                <h3 className="font-serif text-white text-[18px] font-bold mb-2">{title}</h3>
      ...
      {testimonials.length > 0 && (
        <section className="max-w-6xl mx-auto px-5 sm:px-8 py-16 md:py-20">
          <div className="text-center mb-10">
            <p className="text-[#2E9AB8] text-[13px] font-semibold uppercase tracking-widest mb-2">Cerita Mereka</p>
            <h2 className="font-serif text-[#141422] text-3xl md:text-4xl font-bold">Yang Mereka Katakan</h2>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-5 sm:px-8 py-16 pb-24">
        <div className="bg-gradient-to-br from-[#343270] to-[#285090] rounded-3xl p-10 md:p-16 text-center relative overflow-hidden">
          ...
          <div className="relative">
            <p className="text-[#AADDE9] text-[13px] font-semibold uppercase tracking-widest mb-3">Mulai Sekarang</p>
            <h2 className="font-serif text-white text-3xl md:text-4xl font-bold mb-4">
              Siap Menemukan<br />Rumah Impian?
            </h2>
            <p className="text-white/70 text-[15px] mb-8 max-w-md mx-auto leading-relaxed">
              Konsultasikan kebutuhan properti Anda dengan tim kami. Gratis, tanpa komitmen.
            </p>
